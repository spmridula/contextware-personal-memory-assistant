...
const DB_VERSION = 2; // Bumped to 2 to add lastConversation field
...
async function getDB() {
  return openDB(DB_NAME, DB_VERSION, {
    upgrade(db, oldVersion) {
      ...
      // Version 1→2: lastConversation is a new field.
      // IndexedDB is schemaless — existing records simply won't have it;
      // all read paths return '' as fallback, so no data migration needed.
    },
  });
}
...
export async function addFace(entry) {
  try {
    ...
    const faceData = {
      ...
      lastConversation: entry.lastConversation?.trim() || '', // NEW FIELD
...
export async function getAllFaces() {
  try {
    ...
    const processedFaces = faces.map((face) => ({
      ...face,
      lastConversation: face.lastConversation || '', // backward-safe fallback
...
export async function updateFace(id, updates) {
  try {
    ...
    const updatedFace = {
      ...
      lastConversation: updates.lastConversation !== undefined
        ? updates.lastConversation.trim()
        : (existing.lastConversation || ''), // NEW FIELD — backward-safe
      timestamp: Date.now(),
    };
